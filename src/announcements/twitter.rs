...
async fn wait_until(rate_limit_reset: u64) -> Result<(), Error> {
    ...
    let rate_limit_reset_instant = Instant::now() + SystemTime::now().duration_since(rate_limit_reset)?;
    await!(Delay::new(rate_limit_reset_instant).compat())
        .context("failed to wait until the rate-limit is reset")?;
    Ok(())
}

async fn rate_limit<T, Fn: FnMut() -> Fut, Fut: Future<Output = Result<Response<T>, egg_mode::error::Error>>>(mut f: Fn) -> Result<Response<T>, Error> {
    loop {
        match await!(f()) {
            Ok(res) => {
...
async fn init<'a>(config: &'a Config, handle: &'a Handle) -> Result<(Token, HashSet<u64>), Error> {
    ...
    for user in &config.twitter_users {
        let user = await!(rate_limit(|| egg_mode::user::show(user, &token, handle).compat()))
...
async fn inner<'a>(config: &'a Config, pg_pool: &'a PgPool, handle: &'a Handle, token: &'a Token, users: &'a HashSet<u64>) -> Result<(), Error> {
    ...
    for &user_id in users {
        ...
        let last_tweet_id = State::get::<u64, _>(&state_key, &conn)
            .context("failed to get the last tweet ID")?;

        let timeline = egg_mode::tweet::user_timeline(user_id, true, true, token, handle)
            .with_page_size(200);
        ...
        if last_tweet_id.is_some() {
            ...
            for tweet in &tweets {
                ...
                if tweet.in_reply_to_user_id.map(|user_id| users.contains(&user_id)).unwrap_or(true) {
                    config.announcements.say(format_args!(
                        "New tweet from {}: https://twitter.com/{}/status/{}",
                        tweet.user.as_ref().map(|user| &user.name[..]).unwrap_or("???"),
                        tweet.user.as_ref().map(|user| &user.screen_name[..]).unwrap_or("a"),
                        tweet.id,
                    ))
